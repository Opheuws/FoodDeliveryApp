...
public class MenuListAdapter extends RecyclerView.Adapter<MenuListAdapter.MyViewHolder> {
    ...
    public MenuListAdapter(List<Menu> menuList, MenuListClickListener clickListener) {
        ...
        this.clickListener = clickListener; //Evento de clique
    }
    ...
    public void onBindViewHolder(@NonNull MenuListAdapter.MyViewHolder holder, int position) {
        ...
        holder.menuPrice.setText("Preço: $"+menuList.get(position).getPrice());
        ...
        holder.imageMinus.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Menu menu  = menuList.get(position); //Evento de clique ao escolher produto
                int total = menu.getTotalInCart();
                total--;
                if(total > 0 ) {
                    menu.setTotalInCart(total);
                    clickListener.onUpdateCartClick(menu);
                    holder.tvCount.setText(total +""); //Adiciona o valor do produto ao total quando clicar
                } else {
